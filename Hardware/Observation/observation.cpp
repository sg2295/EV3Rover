...
USSensorWrapper::USSensorWrapper(ev3dev::address_type us_motor_add, ev3dev::address_type us_sensor_add) :
  ...
  Error(us_sensor->connected(), "Sensor not connected");
  Error(motor->connected(), "Motor not connected");
  ...
  Error(motor->position() == m_start_pos, "Motor position invalid in ctor.");
}
